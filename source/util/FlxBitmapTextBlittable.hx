 ...
 */
class FlxBitmapTextBlittable extends FlxSprite
{
	/**
	 ...
	 * @param	forceRenderBlit	Force text to be blit rendered.
	 */
	public function new(?font:FlxBitmapFont, ?forceRenderBlit:Bool=false) 
	{
		super();
		
		forceBlit = forceRenderBlit;
	...
	function updateTextBitmap(useTiles:Bool = false):Void 
	{
		...
		else if (FlxG.renderTile && !forceBlit)
		{
	...
	{
		if (!FlxG.renderTile && !forceBlit) return;
